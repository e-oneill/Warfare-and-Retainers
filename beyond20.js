...
function sendCustomEvent(name, data = [
  ...
  const event = new CustomEvent('Beyond20_' + name, {
    'detail': data
  });
...
function alertSettings(url, title) {
  ...
  const img = E.img({
    src: chrome.extension.getURL('images/icons/icon32.png'),
  ...
  get: function (obj, name) {
    return new Proxy(function () {
      ...
      apply: (target, thisArg, argumentsList) =>{
        ...
        for (const [name,
        value]of Object.entries(attributes))
...
class RollType {
  ...
  static get DISADVANTAGE() {
    return 4;
  }
  static get THRICE() {
    return 5;
  }
  static get SUPER_ADVANTAGE() {
    return 6;
  }
  static get SUPER_DISADVANTAGE() {
    return 7;
  }  // If a feat overrides it to have advantage;

  static get OVERRIDE_ADVANTAGE() {
    return 8;
  }
  static get OVERRIDE_DISADVANTAGE() {
    return 9;
  }
}
class CriticalRules {
  ...
  static get HOMEBREW_MAX() {
    return 1;
  }  // Impossible to achieve with Roll20 && hard with RollRenderer because of brutal && other mods.;

  static get HOMEBREW_DOUBLE() {
    return 2
  }
  static get HOMEBREW_MOD() {
    return 3;
  }
  static get HOMEBREW_REROLL() {
    return 4;
  }
...
const options_list = {
  'whisper-type': {
    ...
    'choices': {
      [
        WhisperType.NO.toString()
      ]: 'Never whisper',
      [
        WhisperType.YES.toString()
      ]: 'Always whisper',
      [
        WhisperType.QUERY.toString()
      ]: 'Ask every time'
    }
  },
  'whisper-type-monsters': {
    ...
    'choices': {
      [
        WhisperType.NO.toString()
      ]: 'Use general whisper setting',
      [
        WhisperType.HIDE_NAMES.toString()
      ]: 'Hide monster and attack name',
      [
        WhisperType.YES.toString()
      ]: 'Always whisper monster rolls',
      [
        WhisperType.QUERY.toString()
      ]: 'Ask every time'
    }
  ...
  'roll-type': {
    ...
    'choices': {
      [
        RollType.NORMAL.toString()
      ]: 'Normal Roll',
      [
        RollType.DOUBLE.toString()
      ]: 'Always roll twice',
      [
        RollType.QUERY.toString()
      ]: 'Ask every time',
      [
        RollType.ADVANTAGE.toString()
      ]: 'Roll with Advantage',
      [
        RollType.DISADVANTAGE.toString()
      ]: 'Roll with Disadvantage',
      [
        RollType.THRICE.toString()
      ]: 'Always roll thrice (limited support on Roll20)',
      [
        RollType.SUPER_ADVANTAGE.toString()
      ]: 'Roll with Super Advantage',
      [
        RollType.SUPER_DISADVANTAGE.toString()
  ...
  'critical-homebrew': {
    ...
    'choices': {
      [
        CriticalRules.PHB.toString()
      ]: 'Standard PHB Rules (reroll dice)',
      [
        CriticalRules.HOMEBREW_MAX.toString()
      ]: 'Homebrew: Perfect rolls',
      [
        CriticalRules.HOMEBREW_REROLL.toString()
      ]: 'Homebrew: Reroll all damages'
    }
  },
  ...
  'hotkeys-bindings': {
    ...
    'type': 'special',
    'default': null    // callbacks will be added after the functions are defined
  },
  'roll20-template': {
  ...
  'components-display': {
    ...
    'choices': {
      'all': 'All components',
      'material': 'Only material components',
...
function createHTMLOptionEx(name, option, short = false) {
  ...
  if (option.type == 'bool') {
    ...
    }, title), ...description_p, E.div({
      class : 'material-switch pull-right'
    }, E.input({
      id: name,
      class : 'beyond20-option-input',
      name,
      type: 'checkbox'
    }), E.label({
      for : name,
    ...
    e = E.li({
      class : 'list-group-item beyond20-option beyond20-option-string'
    }, E.label({
      class : 'list-content',
      for : name
    }, E.h4({
    ...
    const dropdown_options = Object.values(option.choices).map(o=>E.li({
    }, E.a({
      href: '#'
    }, o)));
    ...
    e = E.li({
      ...
      id: name,
      class : 'input select beyond20-option-input',
      href: ''
    }, option.choices[option.default]), E.ul({
      class : 'dropdown-menu'
    }, ...dropdown_options), E.i({
      id: `${ name }--icon`,
      class : 'icon select'
    })
    )
    ...
    e = E.li({
      class : 'list-group-item beyond20-option beyond20-option-link'
    }, E.label({
      class : 'list-content',
      id: name
    }, E.a({
    ...
    e = E.li({
      class : 'list-group-item beyond20-option beyond20-option-info'
    }, E.label({
      class : 'list-content',
      for : name,
...
function getVTTTabSetting(name) {
  ...
  'This World',
  'This Specific Tab'].includes(value)) {
    ...
    ret = {
      'id': (['This Campaign',
      'This World'].includes(value) ? 0 : current_tab.id),
      'title': title,
      'vtt': vtt
    }
...
function createDiscordChannelsCombobox(name, description, title, dropdown_options) {
  ...
  for (let option of dropdown_options) {
    const name = option.secret ? E.strong({
    ...
    options.push(E.li(attributes, E.a({
      href: '#'
    }, name)));
  }
  for (let p of description_p)
...
function setDiscordChannelsSetting(name, settings) {
  ...
  if (dropdowns.find(d=>d.secret)) dropdowns.push({
    name: 'Delete selected channel',
    action: 'delete'
  })
  dropdowns.push({
...
function configureHotKey(bindings, bindings_div, html, key) {
  ...
  const onKeydown = (event) =>{
    ...
    const actions = $(`
            <div>
                <div>
                    Select the action to perform when <strong>
    ${ newKey }</strong> is pressed :
                </div>
                <select>
    ...
    for (const action in BINDING_NAMES) {
      ...
      group.append($(`
                <option value="
      ${ action }" ${ bindings[key] === action ? 'selected' : '' }>${ BINDING_NAMES[action] }</option>
    ...
    for (const name in character_settings) {
      ...
      group.append($(`
                <option value="
      ${ action }" ${ bindings[key] === action ? 'selected' : '' }>${ option.title }</option>
  ...
  alertify.dialog('Beyond20HotkeyConfirm', function () {
    return {
    };
  }, false, 'confirm');
...
function addHotKeyToUI(bindings, bindings_div, key) {
  ...
  const html = $(`
        <div style="border-bottom: 1px grey solid; display: flex; justify-content: space-between;">
            <div class="hotkey-event" style="cursor: pointer; flex-shrink: 1; padding: 5px; font-weight: bold;">
  ${ key || '' }</div>
            <div class="hotkey-action" style="cursor: pointer; overflow: hidden; text-overflow: ellipsis; text-align: center; padding: 5px; flex-grow: 1;">
  ${ binding_name }</div>
  ...
  html.find('.delete-hotkey').click(ev=>{
    ...
    if (Object.keys(bindings).length == 0) {
      ...
      bindings_div.find('.bindings-header').css({
        display: 'none'
      });
    }
  });
...
function createHotkeysSetting(name, short) {
  ...
  }, opt.title), ...description_p, E.div({
    class : 'save button-group'
  }, E.button({
    id: name,
    name,
    class : 'beyond20-option-input btn',
    type: 'button',
    'data-bindings': ''
  }, 'Set Hotkeys'), )
  )
  ...
  async handleModifiers() {
    if (this._reroll.active) {
      ...
      for (let roll of this._rolls) {
        ...
        const die = roll.roll;
        if ((this._reroll.operator == '=' && die == this._reroll.value) || (this._reroll.operator == '<=' && die <= this._reroll.value) || (this._reroll.operator == '<' && die < this._reroll.value) || (this._reroll.operator == '>=' && die >= this._reroll.value) || (this._reroll.operator == '>' && die > this._reroll.value)) {
          roll.discarded = true;
          rerolls++;
        }
      }
...
class Beyond20RollRenderer {
  ...
  async queryAdvantage(title) {
    const choices = {
      ...
      [
        RollType.SUPER_ADVANTAGE
      ]: 'Super Advantage',
      [
        RollType.SUPER_DISADVANTAGE
      ]: 'Super Disadvantage'
    }
    const order = [
      RollType.NORMAL,
    ...
    if (lastQuery != advantage) {
      this._mergeSettings({
        'last-advantage-query': advantage
      })
    }
    return advantage;
  }
  ...
  async getToHit(request, title, modifier = '', data = {
    ...
    if ([RollType.DOUBLE,
    RollType.ADVANTAGE,
    RollType.DISADVANTAGE].includes(advantage)) {
    ...
    } else if ([RollType.THRICE,
    RollType.SUPER_ADVANTAGE,
    RollType.SUPER_DISADVANTAGE].includes(advantage)) {
      const roll_1 = this.createRoll(d20 + modifier, data);
      const roll_2 = this.createRoll(d20 + modifier, data);
  ...
  async postDescription(request, title, source, attributes, description, attack_rolls = [
  ], roll_info = [
  ], damage_rolls = [
  ], open = false) {
    ...
    if (request.damages && request.damages.length > 0 && request.rollAttack && !request.rollDamage) {
    ...
    const json_damage_rolls = damage_rolls.map(([l,
    r,
    f]) =>r.toJSON ? [
      l,
      r.toJSON(),
    ...
    const json_total_damages = Object.fromEntries(Object.entries(total_damages).map(([k,
    v]) =>[k,
    v.toJSON ? v.toJSON() : v]));
  ...
  postMessage(request, title, message) {
    ...
    title = '???';
    if (request.sendMessage && this._displayer.sendMessage)
    this._displayer.sendMessage(request, title, message, character, request.whisper, false, '', {
    }, '', [
    ], [
    ], [
    ], [
    ...
    this._displayer.postHTML(request, title, message, character, request.whisper, false, '', {
    }, '', [
    ], [
    ], [
    ], [
  ...
  async rollDice(request, title, dice, data = {
    ...
    await this._roller.resolveRolls(title, [
      roll
    ]);
    return this.postDescription(request, title, null, {
    }, null, [
      roll
    ]);
  }
  async sendCustomDigitalDice(character, digitalRoll) {
    ...
    return this.postDescription(request, digitalRoll.name, null, {
    }, null, digitalRoll.rolls);
  }
  async rollD20(request, title, data, modifier = '') {
    ...
    = await this.getToHit(request, title, modifier, data)
  ...
  rollDeathSave(request, custom_roll_dice = '') {
    return this.rollD20(request, 'Death Saving Throw', {
      'custom_dice': custom_roll_dice
    });
  }
  rollItem(request) {
  ...
  rollTrait(request) {
    ...
    return this.postDescription(request, request.name, source, {
    }, request.description, [
    ], [
    ], [
    ], true);
  ...
  async buildAttackRolls(request, custom_roll_dice) {
    ...
    if (request.rollAttack && request['to-hit'] !== undefined) {
      ...
      = await this.getToHit(request, request.name, to_hit_mod)
    ...
    if (request.rollDamage && request.damages !== undefined) {
      ...
      if (request.name === 'Chromatic Orb') {
        ...
        const ttd_dice = await this.queryGeneric(request.name, 'Is the target missing any of its hit points ?', {
          'd12': 'Yes',
          'd8': 'No'
        }, 'ttd_dice', [
          'd12',
      ...
      for (let i = 0; i < (damages.length); i++) {
        ...
        'Disciple of Life',
        'Temp HP',
        'Alchemical Savant Healing',
        'Enhanced Bond Healing'].includes(dmg_type)) {
        ...
        damage_rolls.push([dmg_type + suffix,
        roll,
        damage_flags]);
        // Handle life transference;
        if (request.name == 'Life Transference' && dmg_type == 'Necrotic') {
          damage_rolls.push(['Healing',
          'Twice the Necrotic damage',
          DAMAGE_FLAGS.HEALING]);
        }
      }
      await this._roller.resolveRolls(request.name, all_rolls)      //Moved after the new resolveRolls so it can access the roll results
      if (request.name.includes('Chaos Bolt')) {
        for (let[i,
        dmg_roll]of damage_rolls.entries()) {
          const [dmg_type,
          roll,
          flags] = dmg_roll;
          if (dmg_type == 'Chaotic energy Damage' && roll.dice[0].faces == 8) {
            const chaos_bolt_damages = [
              'Acid',
              'Cold',
              'Fire',
              'Force',
            ...
            if (Object.keys(damage_choices).length == 1) {
              damage_rolls.push(['Chaotic energy leaps from the target to a different creature of your choice within 30 feet of it',
              '',
              DAMAGE_FLAGS.MESSAGE]);
      ...
      if (is_critical) {
        ...
        for (let i = 0; i < (critical_damages.length); i++) {
          ...
          damage_rolls.push([dmg_type + suffix,
          roll,
          damage_flags | DAMAGE_FLAGS.CRITICAL]);
        }
  ...
  async rollAttack(request, custom_roll_dice = '') {
    const [to_hit,
    damage_rolls] = await this.buildAttackRolls(request, custom_roll_dice);
    const data = {
    }
    ...
    roll_info.push(['Save',
    request['save-ability'] + ' DC ' + request['save-dc']]);
  ...
  rollSpellCard(request) {
    const [source,
    data,
    description] = this.buildSpellCard(request);
    return this.postDescription(request, request.name, source, data, description, [
    ], [
    ], [
    ], true);
  }
  async rollSpellAttack(request, custom_roll_dice) {
    ...
    if (request.range !== undefined)
    roll_info.push(['Range',
    request.range]);
    if (request['cast-at'] !== undefined)
    roll_info.push(['Cast at',
    request['cast-at'] + ' Level']);
    ...
    if (this._settings['components-display'] == 'all') {
      if (components) {
        roll_info.push([prefix || 'Components',
        components]);
      }
    } else if (this._settings['components-display'] == 'material') {
      while (components) {
        if (['V',
        'S'].includes(components[0])) {
        ...
        if (components[0] == 'M') {
          roll_info.push([prefix || 'Materials',
          this._settings['component-prefix'] + components.slice(2, - 1)]);
          components = '';
        }
      }
    }
  ...
  displayAvatar(request) {
    ...
    this._displayer.postHTML(request, request.name, `<img src='${ request.character.avatar }' width='100%'>`, {
    }, character, false, false);
    this.displayAvatarToDiscord(request);
  }
  displayAvatarToDiscord(request) {
    const discordChannel = getDiscordChannel(this._settings, request.character);
    postToDiscord(discordChannel ? discordChannel.secret : '', request, request.name, '', {
    }, '', [
    ], [
    ], [
    ], [
...
class DigitalDice {
  constructor(name, rolls) {
    ...
    for (let dice of this._dice) {
      ...
      dice.rerollDice = async function (amount) {
        ...
        const digital = new DigitalDice(name, [
          fakeRoll
        ])
        await digital.roll();
  ...
  async handleCompletedRoll() {
    ...
    if (this._myResult) {
      ...
      this._myResult.find('.dice_result__info__dicenotation').text(`${ this._rolls.length } roll${ this._rolls.length > 1 ? 's' : '' } sent to VTT`).prepend(E.img({
        src: chrome.extension.getURL('images/icons/icon32.png')
      }))
    }
  }
...
class DigitalDiceManager {
  ...
  static _parseCustomRoll(notification) {
    ...
    const digitalRoll = new DigitalDice(name, [
      roll
    ])
...
dndbeyondDiceRoller.handleRollError = (request, error) =>{
  ...
  if (request.action === 'rendered-roll') {
    return dndbeyondDiceRoller._displayer.postHTML(request.request, request.title, request.html, request.character, request.whisper, request.play_sound, request.source, request.attributes, request.description, request.attack_rolls, request.roll_info, request.damage_rolls, request.total_damages, request.open);
  }
...
function descriptionToString(selector) {
  ...
  return ($(selector).html() || '').replace(/<\/?[^>]+>/gi, '').replace(/&nbsp;/g, ' ').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&apos;/g, '\'').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
}
...
function buildAttackRoll(character, attack_source, name, description, properties, damages = [
], damage_types = [
], to_hit = null, brutal = 0, force_to_hit_only = false, force_damages_only = false) {
  ...
  if (properties['Attack/Save'] !== undefined) {
    const [save_ability,
    save_dc] = properties['Attack/Save'].split(' ');
  ...
  if (damages.length > 0) {
    ...
    if (to_hit) {
      ...
      if (character.hasFeat('Piercer')) {
        for (let i = 0; i < damage_types.length; i++) {
          if (damage_types[i].includes('Piercing')) {
            const piercer_damage = damagesToCrits(character, [
              damages[i]
            ]);
      ...
      if (brutal > 0) {
        ...
        if (highest_dice != 0) {
          let brutal_dmg = `${ brutal }d${ highest_dice }`          // Apply great weapon fighting to brutal damage dice
          if (character.hasClassFeature('Fighting Style: Great Weapon Fighting') && properties['Attack Type'] == 'Melee' && (properties['Properties'].includes('Versatile') || properties['Properties'].includes('Two-Handed'))) {
            brutal_dmg += 'ro<=2'
          }
...
function addRollButton(character, callback, where, {
  small = false,
  append = false,
  prepend = false,
  before = false,
...
= {
  ...
  const button = E.div({
    class : 'ct-beyond20-roll',
    id
  }, E.button({
    class : 'ct-beyond20-roll-button ' + (small ? button_class_small : button_class) + ' ' + rolltype_class
  }, E.img({
    class : 'ct-beyond20-icon',
    src: image ? icon : '',
    style: image ? 'margin-right: 6px;' : ''
  }), E.span({
    class : 'ct-button__content'
  }, text)
  )
  )
  if (append)
...
function addDisplayButton(callback, where, {
  text = 'Display in VTT',
  append = true,
  small = true
}
= {
  ...
  const button = E.div({
    class : 'ct-beyond20-roll-display'
  }, E.button({
    class : 'ct-beyond20-display-button ' + (small ? button_class_small : button_class).replace('filled', 'outline')
  }, E.span({
    class : 'ct-button__content'
  }, text)
  )
...
function addHitDieButtons(rollCallback) {
  ...
  const button = E.div({
    ...
    class : 'ct-beyond20-roll-button ' + button_class_small
  }, E.span({
    class : 'ct-button__content'
  }, 'Roll Hit Die')
  )
...
function addIconButton(character, callback, where, {
  append = false,
  prepend = false,
  custom = false
}
= {
  ...
  const button = E.span({
    class : 'ct-beyond20-' + (custom ? 'custom-roll-button' : 'roll'),
    id,
    style: 'margin-right:3px; margin-left: 3px;'
  }, E.img({
    class : 'ct-beyond20-' + (custom ? 'custom-icon' : 'icon'),
    src: icon
  })
  );
  if (append)
...
function injectDiceToRolls(selector, character, name = '') {
  ...
  $('.ct-beyond20-custom-roll').on('click', (event) =>{
    ...
    sendRoll(character, 'custom', roll, {
      'name': name
    });
  }
  );
...
function beyond20SendMessageFailure(character, response) {
  ...
  if (['roll',
  'rendered-roll'].includes(response.request.action) && (response.vtt == 'dndbeyond' || response.error)) {
...
class Monster extends CharacterBase {
  ...
  parseStatBlock(stat_block) {
    ...
    if (this.type() != 'Creature' && this.type() != 'Extra-Vehicle') {
      ...
      const quick_settings = E.div({
        class : 'ct-beyond20-settings-button',
        style: 'background-color: rgba(0, 0, 0, 0.1)'
      }, E.img({
        class : 'ct-beyond20-settings',
        src: chrome.extension.getURL('images/icons/icon32.png'),
        style: 'vertical-align: top;'
      }), E.span({
        class : 'ct-beyond20-settings-button-label mon-stat-block__tidbit mon-stat-block__tidbit-label',
    ...
    if (avatar.length > 0) {
      ...
      if (avatarImg)
      addDisplayButton(() =>this.displayAvatar(), avatarImg, {
        small: false,
        image: true
      });
    }
    ...
    for (let attr of attributes.toArray()) {
      ...
      if (label == 'Armor Class') {
        ...
        if (add_dice)
        addIconButton(this, () =>this.rollHitPoints(), $(attr).find(base + '__attribute-data-extra'), {
          custom: true
        });
      } else if (label == 'Speed') {
        this._speed = value;
    ...
    for (let ability of abilities.toArray()) {
      ...
      this._abilities.push([abbreviationToAbility(abbr),
      abbr,
      score,
      modifier]);
      if (add_dice) {
        addIconButton(this, () =>this.rollAbilityCheck(abbr), ability, {
          prepend: true
        });
        if (abbr == 'DEX') {
          ...
          if (attributes.length > 0) {
            ...
            if (roll_initiative.length == 0) {
              ...
              initiative = $(E.div({
                class : `${ attribute_prefix } ${ base.slice(1) }__beyond20-roll-initiative`,
                'data-modifier': modifier
              }, E.span({
                class : `${ attribute_prefix }-label`
              }, 'Roll Initiative!'), E.span({
                class : `${ attribute_prefix }-data`
              }, E.span({
                class : `${ attribute_prefix }-data-value`
              }, '  ' + modifier)
              )
              )
              );
    ...
    for (let tidbit of tidbits.toArray()) {
      ...
      if (label == 'Saving Throws') {
        ...
        for (let save of saves) {
          ...
          addIconButton(this, () =>this.rollSavingThrow(abbr), data, {
            append: true
          });
        ...
        if (this.type() == 'Monster') {
          ...
          for (let skill in this._skills) {
            ...
            if (add_dice)
            addIconButton(this, () =>this.rollSkillCheck(skill), data, {
              append: true
            });
          }
        }
  ...
  displayAvatar() {
    sendRoll(this, 'avatar', this.avatar, {
      'name': 'Avatar'
    });
  }
  rollHitPoints() {
  ...
  rollInitiative() {
    for (let ability of this._abilities) {
      if (ability[1] == 'DEX') {
        ...
        sendRoll(this, 'initiative', '1d20' + initiative, {
